...
public class LoginService {
	...
	public UserInfo login() throws IllegalArgumentException, LoginServiceException, ConfigurationException {
		...
		UserInfo userInfoNew = UserInfo.BuildFromJson(userInfo);
		userInfoNew.setUserid(loginResult.getString("id"));
		return userInfoNew;
	}
	...
	public UserInfo check() throws LoginServiceException, ConfigurationException {
		if(JfinalConfig.IS_PRODUCT_ENV) {
			...
			UserInfo userObj = UserInfo.BuildFromJson(userInfo);
			userObj.setUserid(id);
			return userObj;
		}else {
			UserInfo mockUser = new UserInfo();
			mockUser.setUserid("test_userid");
			return mockUser ;
		}
	}
